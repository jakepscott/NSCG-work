...
library(survey)
library(magrittr)
NSCG_2003_raw <- read_rds("Data/NSCG_2003_raw.rds")

#no undergrad debt, no jobins, no benefit vars
NSCG_2003_clean <- NSCG_2003_raw %>% dplyr::select(JOBSATIS, BAYR, OCEDRLP, RACETHM, GENDER, AGE, CTZUSIN, CHLVIN, MARSTA, SPOWK, RESPLOC, HCAPIN, DGRDG, HDCARN, 
                                                   HDPBPR,NDGRMED, SALARY, HRSWK, SUPWK, STRTYR, WAPRI, FACADV,
                                                   FACBEN, FACCHAL, FACIND, FACLOC, FACRESP, FACSAL, FACSEC, FACSOC, EMSIZE, NEDTP, LFSTAT, NRREA, NBAMED,
                                                   NBANED, NOCPR, WTSURVY, HISPCAT, REFYR, COMCOLI, DIFHEAR, DIFLIFT, DIFSEE, DIFWALK,
...
majors_by_job <-  NSCG_2003_clean %>%  dplyr::select(NOCPR, NDGRMED, WTSURVY) %>% group_by(NOCPR) %>% nest()


jobs_by_major <-  NSCG_2003_clean %>% filter(NOCPR!=999989) %>%  dplyr::select(NOCPR, NDGRMED, WTSURVY) %>% group_by(NDGRMED) %>% nest()


##This function takes a dataframe as an input, and outputs a "design" object, which tells svy commands (like svytable) how to weight the data
...
##is next to the design object for it. This will allow us to use weights when we work with them. So the cell for NOCPR==1 and 
## column = design is the weights for the NOCPRs==1 minidataframe. 
majors_by_job <- majors_by_job %>% mutate(design = map(data, design_func))
jobs_by_major <- jobs_by_major %>% mutate(design = map(data, design_func))
...
for (i in seq_along(majors_by_job$NOCPR)) {
  ## create survey table for this job cat
  majors_by_job$majors_tables[[i]] <- svytable(~majors_by_job$data[[i]]$NDGRMED, design = majors_by_job$design[[i]]) %>%
    prop.table() %>%
    as.data.frame() %>%
    as_tibble()
  ## dplyr::rename column in nested survey table
  majors_by_job$majors_tables[[i]] <- dplyr::rename(majors_by_job$majors_tables[[i]], major = majors_by_job.data..i...NDGRMED)
  ...
  majors_by_job$majors_tables[[i]] <- arrange(majors_by_job$majors_tables[[i]], desc(percentage))
  ##make a cumulative sum column of percent for each majors_tables tibble
  majors_by_job$majors_tables[[i]] <- majors_by_job$majors_tables[[i]] %>% mutate(cum_perc = cumsum(majors_by_job$majors_tables[[i]]$percentage))
  ##extract all majors for which the cumulative sum of percentage is less than or equal to 75%, put that in a new mini tibble
  majors_by_job$top_75_p[[i]] <- majors_by_job$majors_tables[[i]] %>% filter(ifelse(test = (percentage<=75), yes = cum_perc <= 75, no = percentage>75))
  ...
  majors_by_job$top_75_p_inclusive[[i]] <- majors_by_job$majors_tables[[i]] %>% mutate(over75=case_when(cum_perc>=75~1,TRUE~0),
...
majors_by_job <- majors_by_job %>% dplyr::select(-data,-design,-majors_tables)

##Arranging so NOCPR==0 is at top
majors_by_job<- majors_by_job %>% arrange(NOCPR)

###**************************************************Now turning to making the mismatch variable**************************************************

##Getting data set up
## added dplyr:: to dplyr::select bc else it conflicts with MASS @Caio
design <- svydesign(~0, data = NSCG_2003_clean, weights = NSCG_2003_clean$WTSURVY)
...
NSCG_2003_clean <- left_join(NSCG_2003_clean, majors_by_job, by = "NOCPR") ##THIS IS THE KEY
...
library("progress")
pb <- progress_bar$new(format = "[:bar] :current/:total (:percent)", total = nrow(NSCG_2003_clean))
for (i in 1:nrow(NSCG_2003_clean)) {
  pb$tick()
  NSCG_2003_clean$Obj_Mismatch[i] <- NSCG_2003_clean[i,]$NDGRMED %in% NSCG_2003_clean[i,]$top_75_p[[1]]$major ##need to do this because the ith row of top_5 major is a list with 
  ##one element, the tibble. RUn this code to see this: class(NSCG_2003_clean[3,]$top_5). So we need to tell r, look at the ith row of the top_5 column,  which will be 
  ##a list, then take the first element of this list, which is a tibble, then look at the major column. 
  NSCG_2003_clean$Obj_Mismatch_Inclusive[i] <- NSCG_2003_clean[i,]$NDGRMED %in% NSCG_2003_clean[i,]$top_75_p_inclusive[[1]]$major
}


##this converts mismatches to numeric
NSCG_2003_clean$Obj_Mismatch <-  as.numeric(NSCG_2003_clean$Obj_Mismatch) 
NSCG_2003_clean$Obj_Mismatch_Inclusive <-  as.numeric(NSCG_2003_clean$Obj_Mismatch_Inclusive) 

##Inverting variables. Right now it is 1 if matched, I want it to be 1 if mismatched
NSCG_2003_clean <- NSCG_2003_clean %>% mutate(Obj_Mismatch = ifelse(Obj_Mismatch == 1, 0, 1),
                                              Obj_Mismatch_Inclusive = ifelse(Obj_Mismatch_Inclusive == 1, 0, 1))

##Getting non-list columns
NSCG_2003_clean <- NSCG_2003_clean %>% select_if(negate(is.list))

###****************************************************************************************************
##Jobs by Major- so looking at what jobs are common are given fields of study
###****************************************************************************************************
jobs_by_major <-  NSCG_2003_clean %>% filter(NOCPR!=999989) %>%  dplyr::select(NOCPR, NDGRMED, WTSURVY) %>% group_by(NDGRMED) %>% nest()
jobs_by_major <- jobs_by_major %>% mutate(design = map(data, design_func))
...
for (i in seq_along(jobs_by_major$NDGRMED)) {
  ## create survey table for this job cat
  jobs_by_major$jobs_tables[[i]] <- svytable(~jobs_by_major$data[[i]]$NOCPR, design = jobs_by_major$design[[i]]) %>%
    prop.table() %>%
    as.data.frame() %>%
    as_tibble()
  ## dplyr::rename column in nested survey table
  jobs_by_major$jobs_tables[[i]] <- dplyr::rename(jobs_by_major$jobs_tables[[i]], job = jobs_by_major.data..i...NOCPR)
  ## add percentage column to  survey table 
  jobs_by_major$jobs_tables[[i]]$percentage <- jobs_by_major$jobs_tables[[i]]$Freq*100
  ## sort survey table by percent
  jobs_by_major$jobs_tables[[i]] <- arrange(jobs_by_major$jobs_tables[[i]], desc(percentage))
  ##make a cumulative sum column of percent for each jobs_tables tibble
  jobs_by_major$jobs_tables[[i]] <- jobs_by_major$jobs_tables[[i]] %>% mutate(cum_perc = cumsum(jobs_by_major$jobs_tables[[i]]$percentage))
  ##extract all majors for which the cumulative sum of percentage is less than or equal to 75%, put that in a new mini tibble
  jobs_by_major$top_75_p[[i]] <- jobs_by_major$jobs_tables[[i]] %>% filter(ifelse(test = (percentage<=75), yes = cum_perc <= 75, no = percentage>75))
  ...
  jobs_by_major$top_75_p_inclusive[[i]] <- jobs_by_major$jobs_tables[[i]] %>% mutate(over75=case_when(cum_perc>=75~1,TRUE~0),
                                                                                     include = case_when(over75==1 & lag(over75)==0~1,
                                                                                                         over75==0~1,
                                                                                                         TRUE~0)) %>% ##Looking at when cumperc turns from below to above 75%
    filter(include==1)
...
jobs_by_major <- jobs_by_major %>% dplyr::select(-data,-design,-jobs_tables)
...
NSCG_2003_clean <- left_join(NSCG_2003_clean, jobs_by_major, by = "NDGRMED") ##THIS IS THE KEY

NSCG_2003_clean <- NSCG_2003_clean %>% as.tibble()
...
for (i in 1:nrow(NSCG_2003_clean)) {
  pb$tick()
  NSCG_2003_clean$Obj_Mismatch_Reversed[i] <- NSCG_2003_clean[i,]$NOCPR %in% NSCG_2003_clean[i,]$top_75_p[[1]]$job ##need to do this because the ith row of top 75 percnt major is a list with 
  ##one element, the tibble. RUn this code to see this: class(NSCG_2003_clean[3,]$top 75 percnt). So we need to tell r, look at the ith row of the top 75 percnt column,  which will be 
  ##a list, then take the first element of this list, which is a tibble, then look at the major column. 
  NSCG_2003_clean$Obj_Mismatch_Reversed_Inclusive[i] <- NSCG_2003_clean[i,]$NOCPR %in% NSCG_2003_clean[i,]$top_75_p_inclusive[[1]]$job
}


##this converts objmatch2 to numeric
NSCG_2003_clean$Obj_Mismatch_Reversed <-  as.numeric(NSCG_2003_clean$Obj_Mismatch_Reversed) 
NSCG_2003_clean$Obj_Mismatch_Reversed_Inclusive <-  as.numeric(NSCG_2003_clean$Obj_Mismatch_Reversed_Inclusive) 


##Right now the mismatch column is 1 if matched, I am reversing it
NSCG_2003_clean <- NSCG_2003_clean %>% mutate(Obj_Mismatch_Reversed = ifelse(Obj_Mismatch_Reversed == 1, 0, 1))
NSCG_2003_clean <- NSCG_2003_clean %>% mutate(Obj_Mismatch_Reversed_Inclusive = ifelse(Obj_Mismatch_Reversed_Inclusive == 1, 0, 1))
...
##Saving
saveRDS(NSCG_2003_clean, file = "Data/NSCG_2003_clean.rds")
