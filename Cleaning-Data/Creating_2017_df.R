...
majors_by_job <-  NSCG_2017_clean %>%  dplyr::select(N2OCPR, NDGRMED, WTSURVY) %>% group_by(N2OCPR) %>% nest()


jobs_by_major <-  NSCG_2017_clean %>% filter(N2OCPR!=999989) %>%  dplyr::select(N2OCPR, NDGRMED, WTSURVY) %>% group_by(NDGRMED) %>% nest()
...
design <- svydesign(~0, data = NSCG_2017_clean, weights = NSCG_2017_clean$WTSURVY)
...
NSCG_2017_clean <- left_join(NSCG_2017_clean, majors_by_job, by = "N2OCPR") ##THIS IS THE KEY
...
library("progress")
pb <- progress_bar$new(format = "[:bar] :current/:total (:percent)", total = nrow(NSCG_2017_clean))
for (i in 1:nrow(NSCG_2017_clean)) {
  pb$tick()
  NSCG_2017_clean$Obj_Mismatch[i] <- NSCG_2017_clean[i,]$NDGRMED %in% NSCG_2017_clean[i,]$top_75_p[[1]]$major ##need to do this because the ith row of top_5 major is a list with 
  ##one element, the tibble. RUn this code to see this: class(NSCG_2017_clean[3,]$top_5). So we need to tell r, look at the ith row of the top_5 column,  which will be 
  ##a list, then take the first element of this list, which is a tibble, then look at the major column. 
  NSCG_2017_clean$Obj_Mismatch_Inclusive[i] <- NSCG_2017_clean[i,]$NDGRMED %in% NSCG_2017_clean[i,]$top_75_p_inclusive[[1]]$major
}


##this converts mismatches to numeric
NSCG_2017_clean$Obj_Mismatch <-  as.numeric(NSCG_2017_clean$Obj_Mismatch) 
NSCG_2017_clean$Obj_Mismatch_Inclusive <-  as.numeric(NSCG_2017_clean$Obj_Mismatch_Inclusive) 

##Inverting variables. Right now it is 1 if matched, I want it to be 1 if mismatched
NSCG_2017_clean <- NSCG_2017_clean %>% mutate(Obj_Mismatch = ifelse(Obj_Mismatch == 1, 0, 1),
                                              Obj_Mismatch_Inclusive = ifelse(Obj_Mismatch_Inclusive == 1, 0, 1))

##Getting non-list columns
NSCG_2017_clean <- NSCG_2017_clean %>% select_if(negate(is.list))
...
NSCG_2017_clean <- left_join(NSCG_2017_clean, jobs_by_major, by = "NDGRMED") ##THIS IS THE KEY

NSCG_2017_clean <- NSCG_2017_clean %>% as.tibble()
...
for (i in 1:nrow(NSCG_2017_clean)) {
  pb$tick()
  NSCG_2017_clean$Obj_Mismatch_Reversed[i] <- NSCG_2017_clean[i,]$N2OCPR %in% NSCG_2017_clean[i,]$top_75_p[[1]]$job ##need to do this because the ith row of top 75 percnt major is a list with 
  ##one element, the tibble. RUn this code to see this: class(NSCG_2017_clean[3,]$top 75 percnt). So we need to tell r, look at the ith row of the top 75 percnt column,  which will be 
  ##a list, then take the first element of this list, which is a tibble, then look at the major column. 
  NSCG_2017_clean$Obj_Mismatch_Reversed_Inclusive[i] <- NSCG_2017_clean[i,]$N2OCPR %in% NSCG_2017_clean[i,]$top_75_p_inclusive[[1]]$job
}


##this converts objmatch2 to numeric
NSCG_2017_clean$Obj_Mismatch_Reversed <-  as.numeric(NSCG_2017_clean$Obj_Mismatch_Reversed) 
NSCG_2017_clean$Obj_Mismatch_Reversed_Inclusive <-  as.numeric(NSCG_2017_clean$Obj_Mismatch_Reversed_Inclusive) 


##Right now the mismatch column is 1 if matched, I am reversing it
NSCG_2017_clean <- NSCG_2017_clean %>% mutate(Obj_Mismatch_Reversed = ifelse(Obj_Mismatch_Reversed == 1, 0, 1))
NSCG_2017_clean <- NSCG_2017_clean %>% mutate(Obj_Mismatch_Reversed_Inclusive = ifelse(Obj_Mismatch_Reversed_Inclusive == 1, 0, 1))
...
NSCG_2017_clean <- NSCG_2017_clean %>% mutate(Undergrad_Debt=case_when(UGLOANR=="2"~ 0,
                                                                       UGLOANR=="3"~5000,
                                                                       UGLOANR=="4"~15000,
                                                                       UGLOANR=="5"~ 25000,
                                                                       UGLOANR=="6"~ 35000,
...
##Saving
saveRDS(NSCG_2017_clean, file = "Data/NSCG_2017_clean.rds")
